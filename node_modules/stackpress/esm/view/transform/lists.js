...
export function generateFieldsetTable(directory, fieldset, column) {
    if (!column.fieldset)
        return;
...
export function generateFieldsetInfo(directory, fieldset, column) {
    if (!column.fieldset)
        return;
    const columnFieldset = column.fieldset;
    const path = `${fieldset.name}/components/lists/${column.title}ListFormat.tsx`;
...
export function generateFormat(directory, fieldset, column) {
    if (typeof column.list.component !== 'string')
        return;
    const path = `${fieldset.name}/components/lists/${column.title}ListFormat.tsx`;
    const source = directory.createSourceFile(path, '', { overwrite: true });

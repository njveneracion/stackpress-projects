"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = upsert;
const create_js_1 = __importDefault(require("./create.js"));
const detail_js_1 = __importDefault(require("./detail.js"));
const update_js_1 = __importDefault(require("./update.js"));
function upsert(model, engine, input, seed) {
    return __awaiter(this, void 0, void 0, function* () {
        const ids = {};
        for (const column of model.ids) {
            if (!input[column.name]) {
                return yield (0, create_js_1.default)(model, engine, input, seed);
            }
            ids[column.name] = input[column.name];
        }
        const row = yield (0, detail_js_1.default)(model, engine, ids, undefined, seed);
        if (row.results) {
            return yield (0, update_js_1.default)(model, engine, ids, input, seed);
        }
        return yield (0, create_js_1.default)(model, engine, input, seed);
    });
}
;
